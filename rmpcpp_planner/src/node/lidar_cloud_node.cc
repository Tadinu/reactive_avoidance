...
LidarCloudNode::LidarCloudNode()
  ...
  server_.setCallback(
      boost::bind(&LidarCloudNode::configCallback, this, _1, _2));
  ...
  pub_pose_ =
      nh_.advertise<trajectory_msgs::MultiDOFJointTrajectory>("trajectory", 1);
...
void LidarCloudNode::goalCallback(const geometry_msgs::Point &pt) {
...
void LidarCloudNode::odometryCallback(const nav_msgs::Odometry &odom) {
  ...
  std::cout << state_W.pos_.transpose() << " " << v_W_.transpose() << std::endl;
  ...
  integrator_.resetTo(T_W_B_.translation(), v_W_);
  for (int i = 0; i < 2; i++) {
    integrator_.forwardIntegrateFixed(full_result, geometry, dt);
    Eigen::Vector3d int_pos = integrator_.getPos();
    Eigen::Vector3d int_vel = integrator_.getVel();
    Eigen::Vector3d int_acc = integrator_.getAcc();
  ...
  msg_markers.markers.push_back(positionToMarker(
      "pos", odom.header.frame_id, T_W_B_.translation(), {0.0, 0.0, 1.0, 0.5}));
  msg_markers.markers.push_back(positionToMarker(
      "goal", odom.header.frame_id, goal_W_, {0.0, 1.0, 0.0, 0.5}));
  pub_debugmarkers_.publish(msg_markers);
}

visualization_msgs::Marker LidarCloudNode::vectorToMarker(
    Eigen::Vector3d vec, std::string name, std::string frame,
    Eigen::Vector3d pos, Eigen::Vector4d color) {
...
visualization_msgs::Marker LidarCloudNode::positionToMarker(
    std::string name, std::string frame, Eigen::Vector3d pos,
